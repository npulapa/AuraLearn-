...
import { predictShape, type Prediction } from "@/lib/shape-predict";
import { saveEntry, SHAPE_VIDEOS } from "@/lib/history";
...
function DrawPage() {
  ...
  async function runPrediction() {
    ...
    const pred = predictShape(allPoints);
    setPrediction(pred);
    setProcessing(false);
    const canvas = canvasRef.current!;
    saveEntry({
      ...
      {prediction && !processing && (
        <div className="mt-6 grid gap-6 lg:grid-cols-2">
          ...
          <div className="glass-card rounded-3xl p-6 animate-fade-in">
            ...
            <h3 className="mt-2 text-xl font-black">Videos about {prediction.shape.toLowerCase()}s</h3>
            <div className="mt-4 grid gap-3">
              {SHAPE_VIDEOS[prediction.shape].map((v) => (
                <a key={v.url} href={v.url} target="_blank" rel="noreferrer" className="flex items-center gap-3 rounded-2xl border border-border bg-card p-3 hover:bg-muted transition">
                  <img src={v.thumb} alt={v.title} className="h-16 w-24 rounded-lg object-cover" />
                  <div className="flex-1 min-w-0">
                    <p className="truncate text-sm font-bold">{v.title}</p>
                    <p className="text-xs text-muted-foreground">YouTube</p>
                  </div>
                  <span className="rounded-full bg-aura-gradient px-3 py-1.5 text-xs font-bold text-white">Watch</span>
                </a>
              ))}
            </div>
          </div>
